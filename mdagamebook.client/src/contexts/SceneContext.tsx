...
interface SceneContextType {
  getSceneData: (
    sceneId: string
  ) => Promise<{ scene: SceneData; links: Link[] }>;
...
export const SceneProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [playerStatsCache, setPlayerStatsCache] = useState<PlayerStats | null>(
    null
  );
  ...
  const [preloadedScenes] = useState<
    Map<string, { scene: SceneData; links: Link[] }>
  >(new Map());
  const { token } = useAuth();
    ...
    async (sceneId: string) => {
      try {
        const response = await fetch(
          `${API_URL}/api/Shops/location/${sceneId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const getMinigameData = useCallback(
    async (sceneId: string) => {
      try {
        const response = await fetch(
          `${API_URL}/api/Minigames/play/${sceneId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const playRPS = useCallback(
    async (minigameId: string, playerChoice: string) => {
      const response = await fetch(
        `${API_URL}/api/Minigames/${minigameId}/play`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
