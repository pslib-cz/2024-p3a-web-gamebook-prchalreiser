...
import { ShopData, Minigame } from "../types";
...
const Scene = () => {
  ...
  useEffect(() => {
    ...
    const loadMinigameData = async () => {
      ...
      if (minigameData) {
        setMinigame(minigameData as unknown as Minigame);
      }
    };
  ...
  const handlePlayRPS = async (minigameId: string, choice: string) => {
    if (!minigame)
      return { playerScore: 0, computerScore: 0, isCompleted: false };
  ...
  const handlePlayNumberGuess = async (numbers: {
    number1: string;
    number2: string;
  }) => {
    if (!minigame) return;
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/Minigames/${minigame.minigameID}/play-numbers`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      await refreshPlayerStats(); // Refresh stats to show new stamina value
      const targetLocationId = result.isCorrect
        ? minigame.winLocationID
        : minigame.loseLocationID;
      navigate(`/scene/${targetLocationId}`);
    } catch (error) {
  ...
  return (
    <SceneTemplate
      ...
      links={currentScene.links as any}
      shop={shop}
      minigame={minigame}
      ...
      onPlayRPS={handlePlayRPS as any}
