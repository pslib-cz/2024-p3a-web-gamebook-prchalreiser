  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < spiralPoints; i++) {
        ...
        const waveOffset = Math.sin(time * 3 + progress * Math.PI * 4) * (currentSize * 0.02) + 
                          (pulseIntensity * currentSize * 0.03 * Math.sin(progress * Math.PI * 8));
        ...
        if (i > 0) {
          ...
          const prevWaveOffset = Math.sin(time * 3 + ((i - 1) / spiralPoints) * Math.PI * 4) * (currentSize * 0.02);
          const prevRadius = ((i - 1) / spiralPoints * maxRadius) + prevWaveOffset;
